...
module.exports = class tester {
	...
	run() {
		...
		for (var i in this.tests) {
			...
			var messageName = this.tests[i].name
			if (globalPassed) globalPassed = passed
			if (!passed) errors.push(messages[messageName])

			results.push({
				passed: passed,
				message: passed ? messages.RESULT_OK : messages[messageName]
			})
		}
	...
	log(results) {
		...
		for (var r in results) {
			...
			if (!last) {
				...
				if (passed) gutil.log(`${this.opts.name}: ` + chalk.green.bold(`${symbols.success} ${message.message}`))
				else gutil.log(`${this.opts.name}: ` + chalk.red.bold(`${symbols.error} ${message.message}`))
			}
		}
	}
}