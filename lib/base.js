	    ...
	    cb();
	  	...
	  	if ('customTests' in opts && opts.customTests) {
	  		for (var i in opts.customTests) {
	  			if (!('test' in opts.customTests[i]) || !('message' in opts.customTests[i]) || !('name' in opts.customTests[i])) continue
	  			if (typeof opts.customTests[i].test === 'function' && typeof opts.customTests[i].message === 'string' && typeof opts.customTests[i].name === 'string') {
	  				var message = {
